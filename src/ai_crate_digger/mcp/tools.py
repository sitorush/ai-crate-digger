...
async def _get_candidate_pool(
    ...
    # Step 1.5: Filter by directory (prefix match on file_path)
    if directory is not None:
        all_tracks = [t for t in all_tracks if str(t.file_path).startswith(directory)]
        ...
        return [
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "directory": {
                            "type": "string",
                            "description": (
                                "Filter to tracks from this directory path "
                                "(prefix match on file_path)"
                            ),
                        },
                        "limit": {
                            "type": "integer",
            ...
            Tool(
                ...
                description=(
                    ...
                    "If no directory is given, scans the default music path (~/Music/mp3)."
                ),
                inputSchema={
                    "type": "object",
                    "properties": {
...
async def _scan_library(settings: Settings, args: dict[str, Any]) -> list[TextContent]:
    ...
    directory = Path(args["directory"]) if args.get("directory") else settings.music_path
